...
use canister_types::models::{api_error::ApiError, boosted::Boosted};

pub struct BoostedStore<'a> {
    store: &'a LocalKey<StorageRef<u64, Boosted>>,
}

impl<'a> BoostedStore<'a> {
    pub fn new(store: &'a LocalKey<StorageRef<u64, Boosted>>) -> Self {
        Self { store }
    }
}
...
impl StorageMethods<u64, Boosted> for BoostedStore<'static> {
    ...
    fn get(&self, key: u64) -> Result<(u64, Boosted), ApiError> {
    ...
    fn get_many(&self, keys: Vec<u64>) -> Vec<(u64, Boosted)> {
        self.store.with(|data| {
    ...
    fn find<F>(&self, filter: F) -> Option<(u64, Boosted)>
    where
        F: Fn(&u64, &Boosted) -> bool,
    {
        self.store
    ...
    fn filter<F>(&self, filter: F) -> Vec<(u64, Boosted)>
    where
        F: Fn(&u64, &Boosted) -> bool,
    {
        self.store.with(|data| {
    ...
    fn insert(&mut self, value: Boosted) -> Result<(u64, Boosted), ApiError> {
        self.store.with(|data| {
            let key = data
                .borrow()
                .last_key_value()
    ...
    /// use `insert` instead
    fn insert_by_key(&mut self, _key: u64, _value: Boosted) -> Result<(u64, Boosted), ApiError> {
    ...
    fn update(&mut self, key: u64, value: Boosted) -> Result<(u64, Boosted), ApiError> {
        self.store.with(|data| {
            if !data.borrow().contains_key(&key) {
...
impl BoostedStore<'static> {
    ...
    /// # Returns
    /// * `Vec<Boosted>` - The boosted if found, otherwise an empty vector
    pub fn get_all(&self) -> Vec<(u64, Boosted)> {
        self.store.with(|data| {
            data.borrow()
                .iter()
