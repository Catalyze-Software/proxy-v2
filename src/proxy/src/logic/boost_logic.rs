...
impl BoostCalls {
    ...
    pub async fn get_boost_by_subject(subject: Subject) -> CanisterResult<(u64, Boost)> {
        let resp = boosteds()
            .find(BoostedFilter::Subject(subject).into())
            .await?
            .ok_or_else(|| ApiError::not_found().add_message("No boosted group or event found"))?;

        Ok(resp)
    }

    pub async fn get_boosts_by_subject(subject: SubjectType) -> CanisterResult<Vec<(u64, Boost)>> {
        boosteds()
